...
| `Marz` | The guarded egress boundary (*border*): the network edge where an effect leaves the machine and becomes irreversible — a per-destination, intent-derived capability, DIFC-checked on export, recorded as an irreversible effect. See [Marz](SUBSYSTEMS.md#marz-guarded-egress). | designed, M1 in progress |
